...
function parse_absence(html) {
    ...
    for (row of rows) {
        ...
        if (!absence_table[course_code]) {
            ...
            if (occurence.type === 'Tutorial') absence_table[course_code].absence.tutorials += 1;
            else absence_table[course_code].absence.lectures += 1;
        } else if (absence_table[course_code].name) {
            absence_table[course_code].absence.occurences.push(occurence);
            if (occurence.type === 'Tutorial') absence_table[course_code].absence.tutorials += 1;
            else absence_table[course_code].absence.lectures += 1;
        }
    }
...
const eval_path = 'Survey%2FSurvey';
async function eval_req(cookie) {
    let res = await fetch(get_url(eval_path), get_request(cookie));
    res = await res.text();
    return res;
}; //TODO
...
async function post_check(cookie) {
    await fetch(post_url(Date.now()), post_request(cookie))
}
